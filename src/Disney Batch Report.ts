 ...
 * Purpose: Automates metadata preparation for song/production sheets.
 ...
 *          - Formats additional sheets as tables
 *
 * Assumptions:
 ...
 *
 * Author: Brandon Johnson (Film & TV Administration, Universal Music Publishing Group)
 * Version: 1.0
 * Date: 2025-08-14
 *
 * Environment:
...
function main(workbook: ExcelScript.Workbook) {
  ...
  // Helper to format a sheet's used range as a table
  const formatSheetAsTable = (sheetName: string, tableName: string) => {
    ...
    const existingTables = workbook.getTables();
    for (let tbl of existingTables) {
  ...
  formatSheetAsTable('AKA Table', 'AKATable');
  formatSheetAsTable('ISRCs for upload', 'ISRCLookup');
  formatSheetAsTable('IP Table', 'IPTable');
}
